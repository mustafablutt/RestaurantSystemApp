  ...
  Widget build(BuildContext context) {
    ...
    int totalPriceOfCart() {
      ...
      for (int i = 0; i < provider.cartList.length; i++) {
        totalCart += provider.cartList[i].price * provider.cartList[i].quantity;
      }
      return totalCart;
    }

    String totalCart = totalPriceOfCart().toString();
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          ...
          for (int i = 0; i < provider.cartList.length; i++) {
            ...
            _firestore.collection('pordersbyuser').doc(_auth.currentUser.uid)
            //     .update({
            //   'list$i':  yourItemList.toList(),
            // });
                .set({
              for (int i = 0; i < provider.cartList.length; i++)
                'list$i': yourItemList.toList(),
